              ...
              allApplicants?.applications.map((user) => (
                <tr key={user._id}>
                  ...
                  <td className="px-4 py-5">
                    {user?.applicant?.createdAt.split("T")[0]}
                  </td>
                  <td className="px-4 py-5">
                    <div className=" px-3 py-1 rounded relative">
                      <HiDotsHorizontal
                      ...
                      <div
                        className={`${
                          open ? "border" : ""
                        } py-1 px-2 absolute top-4.5 left-0`}
                      >
                        {open &&
                          shortListingStatus.map((staus, index) => {
                            return (
                              <div
                                className="cursor-pointer"
                                key={index}
                                onClick={() => statuaUpdate(staus, user._id)}
                              >
